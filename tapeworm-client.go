...
const (
	DnsServerAddress    = "your_public_ip_addr_here:5353" // ip address where your dns tunnel server is
	TunnelDomain        = "subdomain.domain.tld" // your subdomain and tld
...
func main() {
	...
	for i := 0; i < maxRetries; i++ {
		...
		if err == nil {
			...
			defer signal.Stop(winch) // Stop listening for signals when done
			defer close(winch)       // Close the channel
